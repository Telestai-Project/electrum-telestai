...
APPDIR="$BUILDDIR/electrum-tls.AppDir"
...
VERSION=4.5.9
APPIMAGE="$DISTDIR/electrum-tls-$VERSION-x86_64.AppImage"
...
info "installing electrum-tls and its dependencies."
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
...
cp "$PROJECT_ROOT/electrum-tls.desktop" "$APPDIR/electrum-tls.desktop"
cp "$PROJECT_ROOT/electrum_tls/gui/icons/electrum-tls.png" "$APPDIR/electrum-tls.png"
